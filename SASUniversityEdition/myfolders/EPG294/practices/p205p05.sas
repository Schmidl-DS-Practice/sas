...
data highuse(keep=ParkCode ParkName);
	...
	*Add a subsetting IF statement;
	if dayvisits>=5000000; *You must use a subsetting IF statement because the DayVisits column is
							in only one of the tables in the MERGE statement.;
run;
...
data birds_largepark;
	merge birds highuse(in=inhighuse);
	by ParkCode;
	if inhighuse=1;
run;
