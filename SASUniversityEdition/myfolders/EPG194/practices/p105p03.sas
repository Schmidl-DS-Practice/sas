...
proc sgplot data=pg1.np_codelookup;
    where Type in ('National Historic Site', 'National Monument', 'National Park');
    hbar region / group=type seglabel fillattrs=(transparency=0.5) dataskin=crisp; *HBAR statement creates a horizontal bar chart
    																				with separate bars for each Region.
    																				GROUP= option segments each bar by the
    																				distinct values of Type.
    																				seglabel adds label to each segment
    																				fillattr= make bar 50% transparent
    																				dataskin changes color effect on bars;
    keylegend / opaque across=1 position=bottomright location=inside; *KEYLEGEND statement customizes the appearance and
    																	position of the legend.;
    xaxis grid;	*XAXIS statement adds reference lines on the horizontal axis
run;
